...
_TOOL_NAMES = frozenset({"function", "mcp", "patch", "web"})
_TERMINAL_STATES = frozenset({"unknown", "success", "failed"})


def _validate(category: str, tool_name: str, terminal_state: str = "unknown") -> None:
    if category not in _CATEGORIES or tool_name not in _TOOL_NAMES or terminal_state not in _TERMINAL_STATES:
...
def persist_tool_start(
    ...
    started_at: str | None, turn_key: str | None, source_digest: str, source_ordinal: int,
) -> None:
    """Record a safe start fact without overwriting a terminal result."""
    _validate(category, tool_name)
    ...
    if row is None:
        connection.execute(
            """INSERT INTO tool_spans(session_key, call_key, category, terminal_state, latency_ms, tool_name,
               ...
               VALUES (?, ?, ?, 'unknown', NULL, ?, ?, NULL, ?, ?, ?, 'incomplete', 'exact')""",
            (session_key, call_key, category, tool_name, started_at, turn_key, source_digest, source_ordinal),
        )
        return
...
def persist_tool_end(
    ...
    source_digest: str, source_ordinal: int,
) -> None:
    """Record a completion fact, making an end-only span explicit and monotonic."""
    _validate(category, tool_name, terminal_state)
    row = connection.execute(
        "SELECT tool_name, started_at, finished_at, terminal_state, completeness FROM tool_spans WHERE session_key = ? AND call_key = ?",
        (session_key, call_key),
    ).fetchone()
    if row is None:
        span = ToolSpanJoin().end(call_key, finished_at or "")
        connection.execute(
            """INSERT INTO tool_spans(session_key, call_key, category, terminal_state, latency_ms, tool_name,
               started_at, finished_at, turn_key, source_digest, source_ordinal, completeness, provenance)
               VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, ?, 'exact')""",
            (session_key, call_key, category, terminal_state if terminal_state != "unknown" else span.terminal_state,
             latency_ms, tool_name, finished_at, turn_key, source_digest, source_ordinal, span.completeness),
        )
        return
    span = JoinedToolSpan(call_key, row[0], row[1], row[2], row[3], row[4])
